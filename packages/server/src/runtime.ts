...
export async function startApiagexServer(options: StartServerOptions = {}): Promise<StartedApiagexServer | undefined> {
  ...
  const { multiTenant: multiTenantConfig, ...singleTenantConfig } = config;
  const multiTenant = multiTenantConfig
    ? await createRuntimeMultiTenantConfig(multiTenantConfig)
    : undefined;
...
export async function runRuntimeCli(args: string[], options: RuntimeCliOptions = {}): Promise<RuntimeCliResult> {
  ...
  if (command === "tenant") return tenant(args.slice(1));
...
Usage:
  ...
  apiagex tenant     Show tenant provisioning and migration commands.
...
Environment:
  ...
  APIAGEX_MULTI_TENANT    Set true to enable platform/tenant resolution.
  APIAGEX_PLATFORM_DATABASE_PROVIDER sqlite, postgres, or mysql for platform DB.
  APIAGEX_PLATFORM_DATABASE_URL Platform/control database URL.
  APIAGEX_TENANT_SECRET_KEY Base64 32-byte key for encrypted tenant DB URLs.
  APIAGEX_TENANT_ROOT_DOMAIN Root domain for subdomain tenancy.
  APIAGEX_TENANT_PATH_PREFIX Optional local path prefix. Default: /t.
...
function tenant(args: string[]): RuntimeCliResult {
  const subcommand = args[0] ?? "--help";
  if (subcommand === "--help" || subcommand === "-h") return ok(renderTenantHelp());
  if (subcommand === "provision") return ok(renderTenantProvisionHelp());
  if (subcommand === "migrate") return ok(renderTenantMigrateHelp());
  return fail(`Unknown apiagex tenant command: ${subcommand}\n\n${renderTenantHelp()}`);
}

function renderTenantHelp(): string {
  return `apiagex tenant

Usage:
  apiagex tenant provision  Show non-interactive tenant provision inputs.
  apiagex tenant migrate    Show tenant migration inputs.

Environment:
  APIAGEX_MULTI_TENANT=true
  APIAGEX_PLATFORM_DATABASE_PROVIDER=sqlite|postgres|mysql
  APIAGEX_PLATFORM_DATABASE_URL=<platform-db-url>
  APIAGEX_TENANT_SECRET_KEY=<base64-32-byte-key>
`;
}

function renderTenantProvisionHelp(): string {
  return `apiagex tenant provision
...
Runtime secrets stay in environment variables. Generated tenant database passwords are one-time values and must not be committed.
`;
}

function renderTenantMigrateHelp(): string {
  return `apiagex tenant migrate

Modes:
  --slug <slug>     Migrate one tenant.
  --all             Migrate all tenants.
  --dry-run         List migration targets without changing tenant databases.

All-tenant migration should continue safely and report each tenant result.
`;
}
...
async function createRuntimeMultiTenantConfig(config: NonNullable<ReturnType<typeof resolveLocalServerConfig>["multiTenant"]>) {
  const platformDb = await openPlatformDatabase({
    provider: config.platformDatabaseProvider,
    url: config.platformDatabaseUrl,
  });
  await migratePlatformDatabase(platformDb);
  const secretKey = tenantSecretKeyFromBase64(config.secretKeyBase64);
  const cache = new TenantConnectionCache({
    openTenantDatabase: async (tenant) => openRuntimeTenantDatabase(
      tenant.databaseProvider,
      decryptTenantSecret(tenant.databaseUrlEncrypted, secretKey),
    ),
  });
  return {
    enabled: config.enabled,
    getTenantDatabase: (tenant: TenantRecord) => cache.get(tenant),
    getTenantUploadsPath: (tenant: TenantRecord) => tenant.uploadsPath,
    lookupTenant: (lookup: TenantLookup) => findTenant(platformDb, lookup),
    resolver: {
      ...(config.pathPrefix === undefined ? {} : { pathPrefix: config.pathPrefix }),
      ...(config.rootDomain === undefined ? {} : { rootDomain: config.rootDomain }),
    },
    skipPaths: ["/api/platform", "/api/health", "/favicon.ico"],
  };
}

async function openRuntimeTenantDatabase(provider: string, url: string): Promise<ApiagexDatabase> {
  if (provider === "postgres") return openPostgresAdapter(url, { migrate: true });
  if (provider === "mysql") return openMySqlAdapter(url, { migrate: true });
  return openPlatformDatabase({ provider: "sqlite", url });
}
