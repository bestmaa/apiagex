...
MVP note: Settings now owns role configuration with an attached submenu for Admin Roles and Content Roles. Owner/admin control-plane roles are hidden from content API permissions, Content Roles can create/revoke content API tokens, and the Users screen shows the user list before opening a one-API-role create form.
