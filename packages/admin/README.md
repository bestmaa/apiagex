...
MVP note: the current `/adminui` Role screen shows API roles only. Settings has separate Admin Panel Roles and Content API Roles sections; owner/admin control-plane roles are hidden from content API permissions, and the Users screen assigns exactly one API role to each content API user.
...
MVP note: current `/adminui` Role screen sirf API roles dikhata hai. Settings me Admin Panel Roles aur Content API Roles alag sections hain; owner/admin control-plane roles content API permissions se hidden hain, aur Users screen har content API user ko exactly one API role assign karta hai.
