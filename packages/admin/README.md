...
Settings includes a Realtime API screen that enables WebSocket create/update/delete events for selected generated content APIs, shows recent event history, and links to practical session-token, ack, `lastEventId`, reconnect, and refetch docs.
...
Settings me Realtime API screen hai jo selected generated content APIs ke liye WebSocket create/update/delete events enable karta hai, recent event history dikhata hai, aur practical session-token, ack, `lastEventId`, reconnect, plus refetch docs link karta hai.
