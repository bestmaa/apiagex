...
export function SettingsPage() {
  ...
  const [apiRoles, setApiRoles] = useState<RoleRecord[]>([]);
  const [activeRoleId, setActiveRoleId] = useState("");
  const [permissions, setPermissions] = useState<AdminPermissionRecord[]>([]);
  const [status, setStatus] = useState("Settings loading");
  ...
  async function loadSettings(nextActiveRoleId?: string) {
    ...
    if (!result.ok) {
      setStatus(result.error ?? "Settings load failed");
      return;
    }
    const nextAdminRoles = result.adminRoles ?? [];
    setAdminRoles(nextAdminRoles);
    setApiRoles(result.apiRoles ?? []);
  ...
  async function savePermissions() {
    ...
    const result = await saveAdminRolePermissions(activeRole.id, buildPermissionDrafts(permissions));
    setPermissions(result.permissions ?? []);
  ...
  function toggle(action: AdminPermissionAction, allowed: boolean) {
    setPermissions((current) => {
      const exists = current.some((permission) => permission.action === action);
      if (exists) {
        return current.map((permission) =>
          permission.action === action ? { ...permission, allowed } : permission,
        );
      }
      return [...current, { id: `${activeRoleId}:${action}`, roleId: activeRoleId, action, allowed }];
    });
  }
  ...
  return (
    <section aria-labelledby="settings-title">
      ...
      <div className="settings-access-layout">
        ...
        />
        <ApiRoleSection apiRoles={apiRoles} />
      </div>
      <StatusToast title="Settings status">{status}</StatusToast>
    </section>
  );
}
    ...
    <section className="settings-panel" aria-labelledby="admin-role-title">
      ...
      <div className="role-list">
        {props.adminRoles.map((role) => (
          <article className={role.id === props.activeRole?.id ? "role-row is-active" : "role-row"} key={role.id}>
            ...
            <div className="role-row-badges">
              ...
              {role.id === props.activeRole?.id ? <span>Active</span> : null}
            </div>
      ...
      <div className="permission-grid">
        {adminActions.map((action) => (
          <label className={isAllowed(props.permissions, action) ? "permission-toggle is-allowed" : "permission-toggle"} key={action}>
            <input
              checked={isAllowed(props.permissions, action)}
              disabled={locked}
              type="checkbox"
              onChange={(event) => props.onToggle(action, event.target.checked)}
            />
...
function ApiRoleSection({ apiRoles }: { apiRoles: RoleRecord[] }) {
  return (
    <section className="settings-panel" aria-labelledby="api-role-title">
      <h3 id="api-role-title"><KeyRound aria-hidden="true" size={18} /> Content API Roles</h3>
      <p>These roles work only with generated `/api/content` routes.</p>
      <div className="api-role-list">
        {apiRoles.map((role) => <span key={role.id}>{role.name}</span>)}
      </div>
      <a href="#roles">Open API role permissions</a>
    </section>
  );
}

function buildPermissionDrafts(permissions: AdminPermissionRecord[]): AdminPermissionDraft[] {
